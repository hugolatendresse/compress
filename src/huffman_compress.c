...
HuffmanTree* createAndBuildMinHeap(char data[], int freq[], int size);
...
HuffmanTree* createAndBuildMinHeap(char data[], int freq[], int size) {
    // Create a min heap with capacity equal to size
    HuffmanTree* minHeap = createMinHeap(size);

    // Create a new node for each character and insert it into the min heap
    for (int i = 0; i < size; ++i) {
        minHeap->array[i] = newNode(data[i], freq[i]);
    }

    // Set the current size of the min heap
    minHeap->size = size;
...
HuffmanTree* createMinHeap(int capacity) {
    HuffmanTree* minHeap = malloc(sizeof(HuffmanTree));
    minHeap->size = 0;
    minHeap->capacity = capacity;
